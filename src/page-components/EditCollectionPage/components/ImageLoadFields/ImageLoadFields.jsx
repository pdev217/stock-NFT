...
export const ImageLoadFields = ({ className, values, setValues }) => {
  ...
  const handleChange = (file, field) => {
    setValues({ ...values, [field]: { ...values[field], file, link: null } });
  };
  ...
  return (
    <div className={cn(className, styles.wrapper)}>
      ...
      <div className={cn(styles.loadField, styles.loadLogoPlaceholder)}>
        {values.logo.preview && (
          <Image src={values.logo.preview} objectFit="cover" layout="fill" alt="logo" />
        )}
        {values.logo.link && !values.logo.preview && (
          <Image
            src={values.logo.link}
      ...
      <div className={cn(styles.loadField, styles.loadFeaturedPlaceholder)}>
        {values.featured.preview && (
          <Image src={values.featured.preview} objectFit="cover" layout="fill" alt="featured" />
        )}
        {values.featured.link && !values.featured.preview && (
          <Image
            src={values.featured.link}
            loader={({ src }) => `${process.env.BACKEND_ASSETS_URL}/collectionImages/${src}`}
            objectFit="cover"
            layout="fill"
            alt="featured"
        ...
        {!values.featured.link && !values.featured.preview && (
          <Image src="/create-nft/Icon-Image.svg" objectFit="contain" width={48} height={40} alt="featured" />
        )}
        <input
          className={styles.fileInput}
      ...
      <div className={cn(styles.loadField, styles.loadBannerPlaceholder)}>
        {values.banner.preview && (
          <Image src={values.banner.preview} objectFit="cover" layout="fill" alt="banner" />
        )}
        {values.banner.link && !values.banner.preview && (
          <Image
            src={values.banner.link}
            loader={({ src }) => `${process.env.BACKEND_ASSETS_URL}/collectionImages/${src}`}
            objectFit="cover"
            layout="fill"
            alt="banner"
        ...
        {!values.banner.link && !values.banner.preview && (
          <Image src="/create-nft/Icon-Image.svg" objectFit="contain" width={48} height={40} alt="banner" />
        )}
        <input
          className={styles.fileInput}
