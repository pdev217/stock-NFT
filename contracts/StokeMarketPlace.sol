...
contract StokeMarketplace is ReentrancyGuard, Ownable {
	...
	// Start auction    
	function startAuction(uint256 _tokenId, uint256 _price, uint256 _endDay, uint256 _endHours, uint256 _endMins, address _nftContract) external
	{
		...
		require(IERC721(_nftContract).ownerOf(_tokenId) == msg.sender,"You are not owner");
		timeForAuction[_tokenId].time = block.timestamp +(_endDay * uint256(86400)) + (_endHours * uint256(3600)) + (_endMins * uint256(60));
	...
	function buyAuction(uint256 _tokenId) external payable
	{
		...
		require(msg.value >= timeForAuction[_tokenId].minPrice,"amount should be greater");
		require(msg.value > timeForAuction[_tokenId].bidAmount,"previous bidding amount");
		require(timeForAuction[_tokenId].time >= block.timestamp,"auction end");
	...
	function upgradeAuction(uint256 _tokenId, bool choice) external payable
	{
		...
		{
			amountForAuction[_tokenId][msg.sender] += msg.value;
			if(amountForAuction[_tokenId][msg.sender] > timeForAuction[_tokenId].bidAmount)
			{
				timeForAuction[_tokenId].bidAmount = msg.value;
				finalOwner[_tokenId] = msg.sender;
				uint256 values = msg.value;
		...
		{
			require(amountForAuction[_tokenId][msg.sender] > 0,"You dont allow");
	...
	function removeFromAuction(uint256 _tokenId, address _nftContract) external
	{
		require(originalOwner[_tokenId] == msg.sender,"You are not originalOwner");
	...
	function auctionDetail(uint256 _tokenId) external view returns(uint256, address) 
	{
	...
	function listSaleNft(uint256 _tokenId) external view returns(uint256 [] memory, uint256 [] memory, uint256, uint256)
	{
		return (saleNft, auctionNft, timeForAuction[_tokenId].minPrice, nftPrice[_tokenId].price);
	}
}